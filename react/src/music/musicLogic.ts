...
// key -> 12-semitone chromatic scale. Cell `chromIdx` holds the note name for the
// interval whose chromatic index is `chromIdx` (0 = root). Precomputed at build time
// from tonal (`simplify(transpose(key, interval))`) — see scripts/gen_scale_notes.mjs
// — so the runtime carries a static table instead of pulling in @tonaljs/note.
//
// The tritone (b5, chromIdx 6) is spelled as an augmented 4th (sharp/natural) rather
// than a flat, matching the convention players expect on a layout chart. Kept in sync
// with the Perl %scale_notes table.
